...
**Spring Data commons/keyvalue/redis/elasticsearch 已在 fork 侧完成去特征化**，其 fork POM 内部依赖已 fork 化、不再传递官方 `org.springframework:*` 坐标，因此从本 A 类覆盖范围**移除**：三个独立 Data library 块删除，改由 fork `bjca-footstone-bpring-data-bom` 统一纳管，不再声明 exclude。
...
| 组件 | groupId | 模块数 | 备注 |
|------|---------|--------|------|
| Spring GraphQL | `org.springframework.graphql` | 2 | 官方坐标，全部排除 |
...
- **WHEN** 生成 `spring-boot-dependencies` POM
- **THEN** `org.springframework.restdocs:spring-restdocs-asciidoctor` 的 dependencyManagement 条目不含 `org.springframework:*` exclusion
...
- **WHEN** 子模块声明 `org.springframework.kafka:spring-kafka` 且根 `resolutionStrategy` 已配置 Framework / Kafka 映射
