...
`spring-boot-2.7` is a Git repository at `spring-boot-2.7`. This change releases `2.7.18-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.

Representative publications:

`cn.bjca.footstone.bpring.boot:bjca-footstone-bpring-boot:2.7.18-nes.patch.1`

Declared internal dependencies:

`logback`, `reactor-netty-1.0`, `spring-framework-5.3`, `spring-security-5.8`, `spring-authorization-server-0.4`, `spring-data-bom-2.7`, `spring-kafka-2.9`

Explicit exclusions:

`ignored starter and smoke-test projects recorded by the repository release documentation`
...
Only the manifest owner for `spring-boot-2.7` may write to this repository while the component is being prepared.
...
Run representative RELEASE-only consumer validation after local publication. Update `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, `doc/TESTING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.
