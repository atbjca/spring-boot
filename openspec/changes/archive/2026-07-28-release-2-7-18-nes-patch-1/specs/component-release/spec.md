...
The release process MUST assign one active owner to `spring-boot-2.7` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-boot-2.7`.
...
The component SHALL have one dedicated release commit containing `2.7.18-nes.patch.1`, internal RELEASE dependency updates, `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, `doc/TESTING.md`, approved exclusions, and no unrelated source changes.
...
The release process MUST honor and verify these exclusions: `ignored starter and smoke-test projects recorded by the repository release documentation`.
