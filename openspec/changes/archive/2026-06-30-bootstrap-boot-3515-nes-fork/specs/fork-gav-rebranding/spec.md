...
`buildSrc/.../DeployedPlugin.java` MUST 在创建 `MavenPublication` 后检查 `forkArtifactPrefix` 属性，若存在则将 `artifactId` 设为 `project.name.replace("spring-boot", forkArtifactPrefix + "-boot")`。

#### Scenario: 发布制品 artifactId 使用 fork 命名
- **WHEN** 对 `:spring-boot-project:spring-boot-starter-web` 执行 `publishToMavenLocal`
- **THEN** 本地仓库中出现 `cn.bjca.footstone.bpring.boot:bjca-footstone-bpring-boot-starter-web:3.5.15-nes.patch.1-SNAPSHOT`
