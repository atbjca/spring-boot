...
- [x] 4.3 `make clean build-thin` **BUILD SUCCESSFUL**（用户确认）+ `make test` **BUILD SUCCESSFUL**（用户确认）+ `./gradlew --offline :spring-boot-project:spring-boot-dependencies:bomrCheck --console=plain` **BUILD SUCCESSFUL**
...
- [x] 5.1 `doc/NES_GAV_MAPPING.md` / `doc/GAV_MAPPING.md`：新增/补充 Spring Data（4 模块）+ Kafka 的 GAV 映射与规则说明；同步 `doc/REQUIREMENTS.md` 的 Phase C 范围
- [x] 5.2 本 change 以坐标对接和 BOM 解析为主，不新增 CVE 修复；无需更新 `VULNERABILITY_REPORT.md` / CVE 台账

## 6. 归档

- [x] 6.1 归档本 change，spec 增量 sync 到 `openspec/specs/`；`openspec validate --strict` 通过
