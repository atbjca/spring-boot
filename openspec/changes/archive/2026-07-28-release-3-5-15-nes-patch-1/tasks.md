...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-boot-3.5` in `spring-boot-3.5`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `3.5.15-nes.patch.1` without exposing credentials.
...
- [x] 1.5 Confirm all declared upstream components are Nexus-verified RELEASEs and record their evidence: `spring-framework-6.2`, `spring-security-6.5`, `spring-authorization-server-1.5`, `spring-data-bom-3.5`, `spring-kafka-3.3`.
- [x] 1.6 Confirm the approved explicit exclusions and how each will be verified: `Docker-backed system tests and explicitly ignored smoke-test projects documented by the repository`.

## 2. RELEASE preparation and component documentation

- [x] 2.1 Set the component version to `3.5.15-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.boot:bjca-footstone-bpring-boot:3.5.15-nes.patch.1`.
...
- [x] 2.5 Update `README.adoc`, `doc/TESTING.md` with version, GAVs, required RELEASE dependencies, validation commands, Nexus location, exclusions, and notable constraints.
- [x] 2.6 Review the full working diff and confirm it contains no unrelated source changes or credentials.

## 3. Local verification

- [x] 3.1 Audit and reuse development build evidence under the release-only-diff exception; do not rerun `make build-thin`.
- [x] 3.2 Audit and reuse development test evidence under the same exception; do not rerun `make test-gate`.
- [x] 3.3 Generate local publications with `make install` / `publishToMavenLocal -x test` (no clean).
...
- [x] 4.3 Create the dedicated release commit for `spring-boot-3.5` `3.5.15-nes.patch.1`.
...
- [x] 7.1 Confirm `v3.5.15-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [x] 7.2 Create annotated tag `v3.5.15-nes.patch.1` on the exact release commit only after `nexus-verified`.
