...
`spring-boot-dependencies/build.gradle` 中 Spring Security library 条目 MUST 使用 `group(forkGroupIdBase + ".security")` 并通过 `imports` 引入 `${forkArtifactPrefix}-security-bom`。
