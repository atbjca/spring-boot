...
`spring-boot-3.5` is a Git repository at `spring-boot-3.5`. This change releases `3.5.15-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.

Representative publications:

`cn.bjca.footstone.bpring.boot:bjca-footstone-bpring-boot:3.5.15-nes.patch.1`

Declared internal dependencies:

`spring-framework-6.2`, `spring-security-6.5`, `spring-authorization-server-1.5`, `spring-data-bom-3.5`, `spring-kafka-3.3`

Explicit exclusions:

`Docker-backed system tests and explicitly ignored smoke-test projects documented by the repository`
...
- Publish `3.5.15-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v3.5.15-nes.patch.1`.
...
Only the manifest owner for `spring-boot-3.5` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `3.5.15-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
- Build: `make build-thin`
- Tests: `make test-gate`
- Local publication/effective POM: `make install`

Run representative RELEASE-only consumer validation after local publication. Update `README.adoc`, `doc/TESTING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v3.5.15-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v3.5.15-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
