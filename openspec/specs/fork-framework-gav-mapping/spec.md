...
`spring-boot-dependencies/build.gradle` 中 Spring Framework library 条目 MUST 使用 `group(forkGroupIdBase)` 并通过 `bom()` 引入 `${forkArtifactPrefix}-framework-bom`，不再引用 `org.springframework` groupId。
