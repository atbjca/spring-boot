...
## Purpose
TBD - created by archiving change release-3-5-15-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-boot-3.5` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-boot-3.5`.
...
Every generated RELEASE POM or BOM for `3.5.15-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
- **WHEN** `make build-thin`, `make test-gate`, `make install`, the metadata scan, and consumer validation succeed
...
The component SHALL have one dedicated release commit containing `3.5.15-nes.patch.1`, internal RELEASE dependency updates, `README.adoc`, `doc/TESTING.md`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `3.5.15-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v3.5.15-nes.patch.1` on the exact release commit used to build and deploy `3.5.15-nes.patch.1`.
...
The release process MUST honor and verify these exclusions: `Docker-backed system tests and explicitly ignored smoke-test projects documented by the repository`.
