...
Elasticsearch ships https://www.elastic.co/guide/en/elasticsearch/client/java-rest/current/index.html[two different REST clients] that you can use to query a cluster: the low-level client from the NES `cn.bjca.footstone.blasticsearch.client:bjca-footstone-blasticsearch-rest-client` module and the high-level client from the NES `cn.bjca.footstone.blasticsearch.client:bjca-footstone-blasticsearch-rest-high-level-client` module.
The Java packages remain `org.elasticsearch.client.*`.
